...
for (yr in unique(gapminder$year)) { # use for loop to loop thru unique years
  bubble <- gapminder %>%
    filter(year == yr) %>%   # filter to desired year
    ggplot(aes(x = gdpPercap,   # create plot
               y = lifeExp,
               color = continent,
               size = pop)) +
    geom_point() +
  ...
  plot(bubble)    # plot the ggplot graphic
  Sys.sleep(2)    # pause for 2 seconds before doing this again
}
